...
ALTER TABLE `gym_management_system`.`equipment_repair_history` 
DROP FOREIGN KEY `equipment_repair_history_ibfk_1`;
ALTER TABLE `gym_management_system`.`equipment_branchandunits` 
DROP FOREIGN KEY `equipment_branchandunits_ibfk_1`;
...
UPDATE Member_4n_Numbers SET Phone_Number = 752345678 WHERE Member_ID = 'M001' and Phone_Number = 771234567;
UPDATE Member_4n_Numbers SET Phone_Number = 745665234 WHERE Member_ID = 'M005';
DELETE FROM Member_4n_Numbers WHERE Member_ID = 'M006' and Phone_Number = 742345467;
...
UPDATE MANAGER_PHONE SET Phone_Number = 770000000 WHERE Manager_ID = 'MAN001' AND Phone_Number = 771234567;
UPDATE MANAGER_PHONE SET Phone_Number = 770000001 WHERE Manager_ID = 'MAN002' AND Phone_Number = 772345678;
DELETE FROM MANAGER_PHONE WHERE Manager_ID = 'MAN003' AND Phone_Number = 723456789;


UPDATE Schedule_ SET Start_Time = '10:30:00' WHERE Date_ = '2024-03-01' AND Member_ID = 'M001';
UPDATE Schedule_ SET Duration_hrs = 2 WHERE Date_ = '2024-03-01' AND Member_ID = 'M002';
...
FROM member as M
LEFT JOIN SCHEDULE_ as W ON M.Member_ID = W.Member_ID;
...
SELECT M.Member_ID
FROM MEMBER as M
LEFT JOIN SHEDULE_ as W ON M.Member_ID = W.Member_ID
...
FROM 
	(SELECT 
        COUNT(*) as members_per_branch 
    FROM 
        member as m 
    JOIN 
        Trainer as T on m.Trainer_ID = T.Trainer_ID
    JOIN 
        Branch as B on T.Branch_Num = B.Branch_Num
    WHERE 
        T.Experience_yrs >= 5  
        AND B.City = 'Galle'  
    GROUP BY 
        B.Branch_Num) as branch_member_counts;
...
CREATE VIEW Member_Trainer_Details_Complex AS
...
WHERE m.City = 'Galle';
 -- Example condition: Filtering members and trainers from Colombo
...
Select
 ...
 where T.Experience_yrs >= 5 and m.city = 'Galle';
 

/*
 create index  Member_index2 on  Member(city);
  create index  trainerr_index2 on trainer(Experience_yrs);
 */
...
WHERE b.City LIKE 'C%' AND m.First_Name IS NOT NULL;
...
WHERE t.First_Name LIKE '%a%' AND b.City = 'Kandy';
...
WHERE m.first_name LIKE '%n%' 
  AND p.Date_ = '2024-02-03') 
UNION
(SELECT m.member_ID, m.MemType_ID, m.first_name, m.city, p.Time_, p.Date_, p.amount
FROM member AS m
...
CREATE VIEW Membership_Type_Schedule AS
SELECT ID, Type_Name, price, Type
FROM (
    SELECT MemType_ID AS ID, Type_Name, price, 'MS_Type' AS Type
    FROM membership_type
    WHERE Type_Name LIKE '%Gold%' OR Type_Name LIKE '%Platinum%' 
    UNION
    SELECT Member_ID AS ID, CONVERT(Duration_hrs, CHAR), Start_Time, 'Plan' AS Type
    FROM SCHEDULE_
...
-- drop view Membership_Type_Workout_plan;
select * from Membership_Type_Workout_plan;
...
INNER JOIN (
    ...
    FROM TRAINER
    WHERE City = 'Galle'
) AS Galle_Trainers ON Galle_Members.Trainer_ID = Galle_Trainers.Trainer_ID;



-- create index MEMBER_index1 on MEMBER(City);
-- create index TRAINER_index1 on TRAINER(City);
...
create index Member_index1 on Member(city);
create index Member_index1 on Trainer(city);
...
create index MemType_ID_index on MEMBER(MEMTYPE_ID);
create index MEMMER_TYPE_index on MEMBErship_TYPE(TYPE_NAME);
create index EX_YEARS_index on Trainer(Experience_yrs);
...
-----------------------------------------------------------------------------------------------------
explain (SELECT ID, Type_Name, price, Type
FROM (
    SELECT MemType_ID AS ID, Type_Name, price, 'MS_Type' AS Type
    FROM membership_type
    WHERE Type_Name LIKE '%Gold%' OR Type_Name LIKE '%Platinum%' 
...
create index STArtTIME_index on SCHEDULE_(START_Time);
explain (SELECT ID, Type_Name, price, Type
FROM (
    SELECT MemType_ID AS ID, Type_Name, price, 'MS_Type' AS Type
    FROM membership_type
...
--------------------------------------------------------------------------------------------------------
explain (SELECT m.member_ID, m.MemType_ID, m.first_name, m.city, p.Time_, p.Date_, p.amount
FROM member AS m
LEFT JOIN payment AS p 
ON m.member_ID = p.Member_ID
WHERE m.first_name LIKE '%a%' 
  AND p.Date_ = '2024-02-01') 
UNION
(SELECT m.member_ID, m.MemType_ID, m.first_name, m.city, p.Time_, p.Date_, p.amount
FROM member AS m
...
create index firstname_index on MEMBER(first_name);
 create index Date_index on Payment(Date_);
...
WHERE m.first_name LIKE '%a%' 
 ...
 create index tra_FIRSTNAME_index on Trainer(first_name);
...
WHERE m.first_name LIKE '%a%' 
 ...
 ------------------------------------------------------------------------------
 explain (SELECT t.Trainer_ID, t.First_Name AS Trainer_name, b.branch_num,
		t.Experience_yrs, b.Opening_Hours, b.Capacity,
		b.City AS Branch_City
FROM TRAINER t
RIGHT OUTER JOIN BRANCH b 
...
WHERE t.First_Name LIKE '%a%' AND b.City = 'Kandy');
 ...
 create index BRANCH_CITY_index on Branch(city);
...
WHERE t.First_Name LIKE '%a%' AND b.City = 'Kandy');
 ...
 -----------------------------------------------------------------------------------------
 explain (SELECT b.Branch_Num, m.First_Name AS Manager_Name, b.City, b.Town, b.Opening_Hours, b.Capacity,
       b.Street_Name, b.Building_Num
FROM BRANCH b
LEFT OUTER JOIN MANAGER m ON b.Branch_Num = m.Branch_Num
WHERE b.City LIKE 'C%' AND m.First_Name IS NOT NULL);

 CREATE INDEX branch_num_index ON BRANCH(Branch_Num);
 CREATE INDEX branch_MAN_num_index ON MANAGER(Branch_Num);
 explain (SELECT b.Branch_Num, m.First_Name AS Manager_Name, b.City, b.Town, b.Opening_Hours, b.Capacity,
       b.Street_Name, b.Building_Num
FROM BRANCH b
LEFT OUTER JOIN MANAGER m ON b.Branch_Num = m.Branch_Num
WHERE b.City LIKE 'C%' AND m.First_Name IS NOT NULL);
 ...
 CREATE INDEX idx_trainer_trainer_id ON Trainer(Trainer_ID);
 CREATE INDEX trainer_experience_yrs_index ON Trainer(Experience_yrs);
...
--------------------------------------------------------------------------------
 ...
 explain
select AVG(members_per_branch) as average_members_per_branch
FROM 
	(SELECT 
        COUNT(*) as members_per_branch 
    FROM 
 ...
 CREATE INDEX branch_branch_num_index ON branch(Branch_Num);
 CREATE INDEX trainer_branch_indx ON trainer(Branch_Num);
...
FROM 
 ...
 --------------------------------------------------------------------------------
 explain (SELECT M.Member_ID
FROM MEMBER as M
LEFT JOIN SCHEDULE_ as W ON M.Member_ID = W.Member_ID
WHERE W.Member_ID IS NULL);

  CREATE INDEX idx_member_member_id ON MEMBER(Member_ID);
  CREATE INDEX idx_schedule_member_id ON SCHEDULE_(Member_ID);
...
LEFT JOIN SCHEDULE_ as W ON M.Member_ID = W.Member_ID);
   ...
   CREATE INDEX schedule_member_id_index ON SCHEDULE_(Member_ID);
